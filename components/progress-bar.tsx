...
export function ProgressBar({ completedTasks = [] }: ProgressBarProps) {
  ...
  const getCompletionPercentage = () => {
    if (currentStep < 4) {
      // For steps 1-3, use the standard calculation
      return Math.round(((displayStep - 1) / (steps.length - 1)) * 100)
    } else if (currentStep === 4) {
      // For step 4 (Let's Get Started), calculate based on completion tasks
      // Define the same action tasks structure as in completion-screen.tsx
      const totalActionTasks = 5 // add-customer, create-quote, create-job, create-invoice, explore-reports
      const requiredActionTasks = 4 // first 4 are required, last one is optional
      
      // Start at 70% and add up to 30% based on completed tasks
      const baseCompletion = 70
      const maxAdditionalCompletion = 30
      const completionFromTasks = (completedTasks.length / totalActionTasks) * maxAdditionalCompletion
      
      return Math.round(baseCompletion + completionFromTasks)
    } else {
      // For steps beyond 4, return 100%
      return 100
    }
  }
          ...
          <div 
            ...
            style={{ 
              width: currentStep === 4 
                ...
                : `${((displayStep - 1) / (steps.length - 1)) * 100}%` 
            }}
          ></div>
