...
import { JobSummaryInvoiceTutorialScreen } from "./screens/job-summary-invoice-tutorial-screen"
...
export function MainLayout() {
  ...
  const [showSiteGuideFromCustomer, setShowSiteGuideFromCustomer] = useState(false) // Track when to show site guide after customer creation
  ...
  // Reset site guide flag when navigating away from add-site screen
  useEffect(() => {
    if (currentScreen !== "add-site" && showSiteGuideFromCustomer) {
      setShowSiteGuideFromCustomer(false)
    }
  }, [currentScreen, showSiteGuideFromCustomer])
  ...
  const handleSiteSaveSuccessWithTask = () => {
    // Complete site creation task and unlock any related sections
    handleTaskCompleted("add-site")
    setShowSiteGuideFromCustomer(false) // Reset the flag
  }

  const handleSiteSaveSuccessFromOnboarding = () => {
    // Complete site creation task and navigate back to completion screen
    handleTaskCompleted("add-site")
    setShowSiteGuideFromCustomer(false) // Reset the flag
    handleNavigateBackToCompletion()
  }

  const handleNavigateToSiteScreenWithTutorial = () => {
    // Navigate to add site screen and keep wizard mode to ensure proper navigation after save
    setCurrentScreen("add-site")
    setShowSiteGuideFromCustomer(true) // Show guide when coming from customer creation
    // Keep activeTab as "wizard" so that onboarding callbacks are used
    // This ensures the site screen will navigate back to completion after save
  }
  ...
  const handleNavigateToInvoiceTutorial = () => {
    setCurrentScreen("log-job-invoice-tutorial")
  }

  const handleInvoiceJobSaveSuccess = () => {
    setCurrentScreen("job-summary-invoice-tutorial")
  }

  const handleBackToTutorialsFromInvoiceWorkflow = () => {
    setCurrentScreen("wizard")
  }
        ...
        <main className="h-screen pt-16 overflow-auto">
          {currentScreen === "marketplace" ? (
            ...
            <AddCustomerScreen
              ...
              onNavigateToSiteScreen={handleNavigateToSiteScreenWithTutorial}
              showGuide={activeTab === "wizard"}
            />
          ) : currentScreen === "add-site" ? (
            <AddSiteScreen
              onSiteSaveSuccess={activeTab === "wizard" ? handleSiteSaveSuccessFromOnboarding : handleSiteSaveSuccessWithTask}
              onNavigateBackToTutorials={activeTab === "wizard" ? handleNavigateBackToCompletion : handleNavigateBackToTutorials}
              showGuide={activeTab === "tutorials" || showSiteGuideFromCustomer}
            />
          ) : currentScreen === "add-asset" ? (
            <AddAssetScreen
              onAssetSaveSuccess={() => {}}
            ...
            />
          ) : currentScreen === "log-job-invoice-tutorial" ? (
            <LogJobInvoiceTutorialScreen 
              onJobSaveSuccess={handleInvoiceJobSaveSuccess}
              onNavigateBackToTutorials={handleBackToTutorialsFromInvoiceWorkflow}
            />
            ...
            >
              <OnboardingFlow 
                ...
                onNavigateToInvoiceTutorial={handleNavigateToInvoiceTutorial}
              />
            </OnboardingProvider>
          )}
        </main>
